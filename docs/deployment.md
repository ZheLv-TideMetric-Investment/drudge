...
以下是 **2026-09-07 时间与手机窄版简报发布后**的已验证结果，不是持续监控。下一次操作前必须重新确认动态状态。

| 项目 | 验收结果 |
| --- | --- |
| 应用提交 | `68e1a72d2353d2b3bd4acff034e715fc4a6281f1` |
| 应用 tree | `5a75303857ea5b70722588c5941d42dcad8c544e` |
| 发布记录 | `DRUDGE-DEPLOY-68e1a72-20260907` |
| 展示与投递 | `plain-3`：恢复已有时间，图片逻辑宽 480px，PNG 宽 960px；事件 18px、历史 14px，保留原重点与全部事件；旧 SVG 路径继续可读 |
| 代码位置 | 应用提交已同步 GitHub main 和 Tide；后续纯文档提交与运行应用版本分开记录 |
| 进程 | 四个既有 PM2 进程 online，12:21:28（北京时间）仅重启 web-app / web-scheduler，PID 为 2859579 / 2859591；PM2 状态已保存，两个 worker PID/启动时间未变 |
| PNG | 两份既有快照返回 HTTP 200、`image/png`、plain-3 ETag 和真实 PNG 字节；模拟图 960×356 / 20472 字节，与本地生成结果逐字节一致；另一份 960×1820 / 271687 字节 |
| 路由 | 首页、简报健康、两份既有 H5 和旧 SVG 均为 200，无认证挑战；PNG 非法页码和不存在快照为 404 |
| 浏览器 | 公网既有模拟 PNG 在 390px 浏览器实际显示，图片加载成功，无页面异常或失败请求 |
| 只读 API | 公网监控四项可用性均为 true；三个应用健康端点为 200 |
| 通知与配置 | 有效通知保持开启，单收件人合法；Web 两进程读取根配置，构建/运行 Host 均匹配 drudge.microzj.com |
| 数据与入口 | 没有快照迁移或数据清理；Neo4j 容器 ID 与启动时间未变；配置、Home Ingress、认证和调度规则未修改 |

发布前本地验证：73 个套件、877 项测试，完整 `pnpm run verify` 成功；1/3/10/40 条合成内容生成 6 张真实 PNG，文字和页码无越界或重叠，390px/320px 浏览器无横向溢出。Home Ingress manifest 校验通过。Tide 执行锁文件安装、Web 构建与渲染库/字体打包检查，standalone 的 Noto Sans SC 字体及许可与源文件一致；只有 Web 渲染代码变化，没有重建或重启 worker。

本次由用户明确授权发布已经预览的时间与窄版改动。此前 PNG 兼容修复的一条授权模拟消息，用户已确认“图片是正常的”；本轮只做本地、公网与浏览器验收，未额外调用 AI 或真实投递，新版在钉钉手机内的阅读体验尚待观察。已发送消息中的图片列表、URL 和客户端缓存不会自动替换。

### 本次回退材料

- Tide `pve-pre-deploy-68e1a72` → `1af99a9c74e944563c11574cc35c56df9b31b6a0`，该基线运行应用为 `7dd0331`。
- 本次没有 schema 或数据迁移。需要回退时，使用该基线，按下文锁文件安装、重建 Web 和命名重启；不删除已发送快照。
- 回退恢复 `plain-2` 的 720px 宽排版，仍保留服务端 PNG 与中文字体，不退回 SVG 消息。
...
本次原始验收记录位于本地忽略目录 `artifacts/DRUDGE-DEPLOY-68e1a72-20260907.md`；正式结论不依赖该目录可用。
...
- 公网检查工作台、只读查询、简报 H5、PNG 与旧 SVG；PNG 检查真实文件签名、Content-Type、字体和图片内容，浏览器检查实际页面和 Console。只有动到路由时才验证 Caddy/reload 与域名退出结果。
