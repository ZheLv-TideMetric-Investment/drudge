...
以下是 **2026-09-06 PNG 手机端兼容修复发布后**的已验证结果，不是持续监控。下一次操作前必须重新确认动态状态。

| 项目 | 验收结果 |
| --- | --- |
| 应用提交 | `7dd03315c1058eea686bfc529a8c66dbdf404b69` |
| 应用 tree | `85ac5802c7e65d629428950d3ddbb0155896ae7c` |
| 发布记录 | `DRUDGE-DEPLOY-7dd0331-20260906` |
| 展示与投递 | 保持 `plain-2` 版式；新消息改用 `image.png`，服务端双倍像素渲染，中文字体随应用携带；旧 SVG 路径继续可读 |
...
| PNG | 两份既有快照返回 HTTP 200、`image/png` 和真实 PNG 字节；模拟图 1440×276 / 18156 字节，与本地生成结果逐字节一致；另一份 1440×1166 / 250065 字节 |
...
发布前本地验证：73 个套件、872 项测试，完整 `pnpm run verify` 成功；1/3/10/40 条合成内容生成 5 张真实 PNG，最大页 450443 字节，390px 浏览器无横向溢出。standalone 的 Noto Sans SC 字体及许可与源文件一致，Home Ingress manifest 校验通过。Tide 执行锁文件安装、Web 构建与渲染库/字体打包检查；只有 Web 代码和其专用依赖变化，没有重建或重启 worker。

此次修复由用户报告“手机端灰块、详情正常”触发。源站 SVG 200 不能证明钉钉原生图片组件支持 SVG，浏览器检查也不能替代手机客户端验收。**本轮未手动调用 AI 或投递测试消息，钉钉手机实际显示仍待用户确认。** 已发送消息中的 SVG URL 与客户端缓存不会自动换成 PNG。

### 本次回退材料

- Tide `pve-pre-deploy-7dd0331` → `d13f6a539ba05573db797b8db69bc7557ec38cac`，该基线运行应用为 `41972d8`。
- 本次没有 schema 或数据迁移。需要回退时，使用该基线，按下文锁文件安装、重建 Web 和命名重启；不删除已发送快照。
- 回退会让新消息再次使用 SVG，不能把它当作已解决手机兼容问题。
...
本次原始验收记录位于本地忽略目录 `artifacts/DRUDGE-DEPLOY-7dd0331-20260906.md`；正式结论不依赖该目录可用。
