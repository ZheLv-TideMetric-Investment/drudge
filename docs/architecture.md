...
主动消息的职责分为业务格式化、快照存储、图片/H5 渲染和钉钉传输。先保存完整快照再发送图片与链接。图片用于快捷播报：每条事件句加可选的一句已有历史/背景；详细正文、实体清单和原文入口留在 H5，不为图片增加图谱探索能力。线上仍为紧凑快讯版 `quick-2`；本地 `plain-2` 是小字文字列表，用可选的 `emphasis` 保留总结中的核心短语标记，供图片轻量着色加粗。旧快照缺少标记时保持普通文字，不迁移旧数据。字段、代码与验收见[消息手册](dingtalk-briefing.md)。
