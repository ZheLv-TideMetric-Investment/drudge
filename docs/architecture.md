...
主动消息的职责分为业务格式化、快照存储、图片/H5 渲染和钉钉传输。先保存完整快照再发送图片与链接。图片用于快捷播报：每条事件句加可选的一句已有历史/背景，按紧凑信息块排版；详细正文、实体清单和原文入口留在 H5，不为图片增加图谱探索能力。现有总结格式中的事件、时间与历史在图片中分层，快照不变。图片分页与紧凑快讯版已于 2026-09-06 发布；字段、代码与验收见[消息手册](dingtalk-briefing.md)。
