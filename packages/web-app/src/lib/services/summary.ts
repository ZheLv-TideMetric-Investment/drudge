...
############################################################
◆ 二、着重与标记规则  
- 每条只用 **…** 加粗最核心的动作或变化短语，通常一处；有两项同等重要的并列变化时最多两处。
- 动作与关键结果一起标记，例如 **下调政策利率25个基点**、**拟投资12亿元**、**否认裁员传闻**。保留并一同标记“拟、预计、否认、尚未”等决定事实含义的限定。
- 不单独加粗公司名、机构名、普通数字或整句话；历史和时间不加粗。不使用 Emoji、HTML 或颜色标签。

############################################################
◆ 三、Markdown 输出模板  
### 新闻内容  
- 主体**核心动作与变化** *(HH:MM)* [历史：已有背景]
- …  

############################################################
◆ 四、硬性排版规范  
* 列表符统一 - ；每条用一个事件句陈述事实，尽量 ≤ 40 字，以说清事件为先。
* 每条的事件句先独立说清主体、动作与关键变化；不要用机构清单或话题名代替事件。保留“拟、预计、据称、尚未”等确定性限定，不把计划写成已经发生。
* 时间统一用 *斜体(HH:MM)*；跨日则 *YYYY‑MM‑DD HH:MM*。  
* **数字原样输出**（不转中文大写、不加千位分隔符）。  
* 若某条新闻含历史背景，则正文后紧跟  
  [历史：关键脉络 ≤ 30 字]；无历史信息时省略方括号。  
* 全文中文；不加其他装饰；禁止评论、预测或情绪化字眼。`;
}
