...
import entityExtractionService from '../../domain/services/entityExtractionService';
import { NewsExtractionResult } from '../../domain/entities/index';
import { NewsItem, BatchResult, BatchSummary, BatchCallbackData, EntityExtractionResult } from '../../shared/types/common';
...
class NewsProcessingService {
  ...
  private async _processBatchesWithCallback(
    ...
    return { summary }; // 使用回调时不返回具体结果，节省内存
  }

  /**
   * 处理单个批次
   */
  private async _processSingleBatch(batch: NewsItem[], currentBatch: number, totalBatches: number): Promise<BatchResult[]> {
