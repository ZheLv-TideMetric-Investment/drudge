...
请以JSON格式返回：
{
   ...
   * 生成摘要
   */
  async generateSummary(request: SummaryRequest): Promise<LLMJsonResponse<SummaryResponse>> {
    const stylePrompts = {
...
请以JSON格式返回：
{
   ...
   * 批量处理新闻级别评估
   */
  async batchEvaluateNewsLevel(newsItems: Array<{title: string, content: string}>): Promise<LLMJsonResponse<NewsLevelResult>[]> {
    const results: LLMJsonResponse<NewsLevelResult>[] = [];
