...
class NewsLevelChecker {
  ...
  async checkNewsLevels(limit = 50) {
    try {
      ...
      // 获取最新新闻
      const latestNews = await this.storage.getAll();
      ...
      // 限制处理数量
      const newsToProcess = latestNews.slice(0, limitNum);

      console.log(`📊 将检查 ${newsToProcess.length} 条新闻的等级`);
  ...
  async rescanNews(limit = 100) {
    try {
      ...
      // 限制处理数量
      const newsToProcess = latestNews.slice(0, limitNum);
      console.log(`🔄 将重新扫描 ${newsToProcess.length} 条新闻的等级`);
