...
import schedulerManager from './interfaces/schedulers/schedulerManager';
import workerManager from './interfaces/schedulers/workerManager';
import webhookService from './infrastructure/external/WebhookService';
import moment from 'moment-timezone';
import config from './shared/config/config';
import { NewsLevelDescription } from './shared/types/enums';
...
class NewsProcessingSystem {
   ...
   * 设置调度器消息监听
   */
  private setupSchedulerMessageListening(): void {
    // 重写 schedulerManager 的 notifyMainThread 方法来处理消息
    const originalNotifyMainThread = (schedulerManager.constructor.prototype as any).notifyMainThread;
    (schedulerManager as any).notifyMainThread = (type: string, data: any) => {
   ...
   * 处理触发新闻处理请求
   */
  private async handleTriggerNewsProcessing(data: any): Promise<void> {
    if (config.workers.enabled && (workerManager as any).initialized) {
      try {
   ...
   * 错误通知
   */
  async sendErrorNotification(error: Error | string, context: string): Promise<void> {
...
async function startSystem(): Promise<void> {
  try {
    ...
    await system.sendErrorNotification(error as Error, '系统启动失败');
    process.exit(1);
  }
}
