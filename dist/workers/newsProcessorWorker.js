...
class NewsProcessorWorker {
    ...
    async startProcessing(options = {}) {
        ...
        const { useFileWatcher = true, intervalCheck = true, checkInterval = 30000 // 30秒检查一次
         } = options;
    ...
    async processUnhandledNews() {
        ...
        try {
            ...
            logger.info(`新闻处理完成：处理${processedCount}条，跳过${skippedCount}条，错误${errorCount}条，` +
                ...
                for (const graphResult of batchInfo.batchResults) {
                    ...
                    if (graphResult.success && !graphResult.skipped && newsItem) {
                        ...
                        if (graphResult.extractionResult) {
                            try {
                                levelResult = await newsLevelService.checkAndHandleNewsLevel(newsItem, graphResult.extractionResult);
                                // 如果是高级别新闻，立即通知主线程
