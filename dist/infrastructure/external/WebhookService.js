...
class WebhookService {
    ...
    async sendMessage(startTime, endTime, content, title = null) {
        ...
        const results = await Promise.all(tokens.map(async (token) => {
            try {
                const response = await axios.post(`https://oapi.dingtalk.com/robot/send?access_token=${token}`, message, {
                    headers: {
                ...
                logger.error(`钉钉消息发送失败 (token: ${token.substring(0, 8)}...): ${response.data.errmsg}`);
                return false;
            }
            catch (error) {
