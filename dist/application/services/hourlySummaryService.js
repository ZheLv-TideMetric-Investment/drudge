...
重要事件：
${summary.top_events.map((event, index) => `${index + 1}. ${event.event} (重要性: ${event.significance})`).join('\n')}

活跃公司：
${summary.top_companies.map((company, index) => `${index + 1}. ${company.company} (${company.count}次提及)`).join('\n')}
          `,
                },
            ];
    ...
    async sendHourlySummaryNotification(summary) {
        try {
            ...
            await webhookService.sendMessage(moment(summary.hour_start).format('YYYY-MM-DD HH:mm:ss'), moment(summary.hour_end).format('YYYY-MM-DD HH:mm:ss'), notification, 'HOURLY_SUMMARY');
    ...
    async getHistoricalSummaries(startDate, endDate, limit = 24) {
        try {
            ...
            while (current.isBefore(end) && summaries.length < limit) {
                ...
                try {
                    const summary = await knowledgeGraphService.getHourlySummary(hourStart.toISOString(), hourEnd.toISOString());
                    if (summary.total_news_count > 0) {
    ...
    async getTodaySummaryStats() {
        try {
            ...
            const todaySummaries = await this.getHistoricalSummaries(today.toDate(), tomorrow.toDate(), 24);
            ...
            // 获取最活跃的小时
            const mostActiveHour = todaySummaries.reduce((max, current) => current.total_news_count > max.total_news_count ? current : max, { total_news_count: 0 });
            // 获取所有涉及的公司
