...
import newsApiService from '../infrastructure/external/NewsApiService.js';
...
class NewsAcquisition {
    ...
    async getNewsCount() {
        try {
            ...
            // 按时间统计
            const now = moment();
            const today = allNews.filter(item => moment(item.time * 1000).isSame(now, 'day')).length;
            const thisWeek = allNews.filter(item => moment(item.time * 1000).isSame(now, 'week')).length;
            const thisMonth = allNews.filter(item => moment(item.time * 1000).isSame(now, 'month')).length;
            console.log('📊 新闻数量统计:');
    ...
    async cleanOldNews(days = 30) {
        try {
            ...
            const allNews = await this.storage.getAll();
            const oldNews = allNews.filter(item => moment(item.time * 1000).isBefore(cutoffTime));
            if (oldNews.length === 0) {
