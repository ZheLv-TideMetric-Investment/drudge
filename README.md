...
- 2026-09-07 已发布：**“牛长婷”时间与手机窄版简报（`plain-3`）**。逻辑宽 `480px`，PNG 宽 `960px`；补充已有时间，白底、事件 `18px`、历史 `14px`，核心动作/变化保持灰绿色加粗。全部事件进入图片，按需换行和分页；完整正文、实体列表和原文链接留在详情。PNG 手机兼容性此前已由用户通过一条授权模拟消息确认，新版布局已通过公网与浏览器验收。
- 当前约定：保持“一句事件 + 可选已有历史”的小字列表，不继续扩展侧栏、图解、多栏或额外装饰。新总结的重点标记随快照保存，旧简报无标记时保持普通文字。状态和验证范围见[消息手册](docs/dingtalk-briefing.md#下一阶段)。
- 最新改动已同步 GitHub 与 Tide，定时通知保持开启。生成时间、简报时段和条目时间各自取已有字段，不编造缺失时间；本地手机预览最多 `320px`，钉钉显示尺寸由客户端控制。本轮未额外真实投递，新版在钉钉手机内的阅读体验尚待观察。
