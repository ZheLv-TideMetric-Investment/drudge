...
- 2026-09-08 已发布：用户实际使用后决定停止图片播报，机器人换回**原生 Markdown 文字**。应用提交 `43f7449` 已同步 GitHub 与 Tide，定时通知保持开启。
- 内容保持“一句事件 + 可选已有历史”，保留时间和显式重点加粗，全部事件直接进入正文；完整正文、实体清单、来源与原文链接留在详情。字体和宽度由钉钉控制，不继续图片设计迭代。
- 旧 PNG/SVG 和已发快照继续可读，历史消息不会自动转成文字。此次没有新增模型调用或手动测试投递；验收范围见[消息手册](docs/dingtalk-briefing.md#已完成的验收)。
