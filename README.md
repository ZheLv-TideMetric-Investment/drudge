...
- 2026-09-06 已发布：**“牛长婷”小字重点简报（`plain-2`）**。白底、事件 `18px`、历史 `16px`，仅对核心动作/变化使用灰绿色加粗；全部事件进入图片，按需分页。完整正文、实体列表和原文链接留在详情。当天已发布手机兼容修复：新消息改用服务端生成的 PNG，版式保持不变，钉钉手机实际显示仍待用户确认。
